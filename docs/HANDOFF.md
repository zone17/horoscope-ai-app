...
1. **Agent definitions** — System prompts for `daily-publisher`, `social-poster`, `onboarding-guide` agents that know which MCP tools they have
2. **Social accounts** — TikTok, X account setup + Ayrshare connection (Facebook Page already live)
3. **Tests** — `cache:keys`, `content:format`, `assign-daily` are highest risk-to-effort ratio
4. **P2 backlog** — Council rotation sign variance, share card overlap, spurious fetch
5. **Delete remaining old utils** — Migrate last consumers of horoscope-generator, horoscope-service, feature-flags
