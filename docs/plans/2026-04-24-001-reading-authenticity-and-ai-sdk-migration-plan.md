...
| **1e — Self-critique pass (composition, not fold-in)** | **✅ Shipped** | #67 | `reading:generate-with-critique` composes `reading:generate` + `reading:judge` at the call site. Threshold: overall ≤ 3 OR antiBarnum ≤ 3 OR voiceAuthenticity ≤ 3 (anti-template dropped per baseline data — Sonnet 5.00 across 36 cells). Bounded 2 critique rounds. Verb is shipped + tested; cron migration to use it is gated on the function-timeout fix (HANDOFF §15 #7). |
