    ...
    <div className="mt-8 rounded-xl border border-white/10 bg-white/5 backdrop-blur-md p-5">
      ...
      <form onSubmit={handleSubmit} className="flex gap-2">
        <input
          ...
          className="flex-1 rounded-lg bg-white/5 border border-white/10 px-3 py-3 text-base text-white placeholder-white/30 focus:outline-none focus:border-indigo-400/50 transition-colors font-light min-h-[44px]"
        />
        <button
          type="submit"
          disabled={status === 'loading'}
          className="px-4 py-3 rounded-lg bg-indigo-500/20 border border-indigo-400/30 text-sm text-indigo-200 hover:bg-indigo-500/30 transition-colors disabled:opacity-50 font-medium min-h-[44px]"
        >
          {status === 'loading' ? (
